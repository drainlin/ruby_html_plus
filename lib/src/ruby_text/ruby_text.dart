import 'dart:math';

import 'package:flutter/widgets.dart';
import 'package:flutter_hooks/flutter_hooks.dart';
import 'package:tuple/tuple.dart';

import 'ruby_text_data.dart';

class RubySpanWidget extends HookWidget {
  final bool fullOverlay;

  const RubySpanWidget(this.data, {super.key, required this.fullOverlay});

  final RubyTextData data;

  @override
  Widget build(BuildContext context) {
    final TextDirection? textDirection = Directionality.maybeOf(context);
    final TextStyle defaultTextStyle = DefaultTextStyle.of(context).style;
    final bool boldTextOverride = MediaQuery.boldTextOf(context);

    final Tuple2<TextStyle, TextStyle> result = useMemoized(() {
      // text style
      TextStyle? effectiveTextStyle = data.style;
      if (effectiveTextStyle == null || effectiveTextStyle.inherit) {
        effectiveTextStyle = defaultTextStyle.merge(effectiveTextStyle);
      }
      if (boldTextOverride) {
        effectiveTextStyle = effectiveTextStyle.merge(
          const TextStyle(fontWeight: FontWeight.bold),
        );
      }
      assert(effectiveTextStyle.fontSize != null, 'must be has a font size.');
      final TextStyle defaultRubyTextStyle = effectiveTextStyle.merge(
        TextStyle(fontSize: effectiveTextStyle.fontSize! / 1.5),
      );

      // ruby text style
      TextStyle? effectiveRubyTextStyle = data.rubyStyle;
      if (effectiveRubyTextStyle == null || effectiveRubyTextStyle.inherit) {
        effectiveRubyTextStyle = defaultRubyTextStyle.merge(
          effectiveRubyTextStyle,
        );
      }
      if (boldTextOverride) {
        effectiveRubyTextStyle = effectiveRubyTextStyle.merge(
          const TextStyle(fontWeight: FontWeight.bold),
        );
      }

      // spacing
      final String? ruby = data.ruby;
      final String text = data.text;
      if (ruby != null &&
          effectiveTextStyle.letterSpacing == null &&
          effectiveRubyTextStyle.letterSpacing == null &&
          ruby.length >= 2 &&
          text.length >= 2) {
        final double rubyWidth = _measurementWidth(
          ruby,
          effectiveRubyTextStyle,
          textDirection:
              data.textDirection ?? textDirection ?? TextDirection.ltr,
        );
        final double textWidth = _measurementWidth(
          text,
          effectiveTextStyle,
          textDirection:
              data.textDirection ?? textDirection ?? TextDirection.ltr,
        );

        if (textWidth > rubyWidth) {
          final double newLetterSpacing = (textWidth - rubyWidth) / ruby.length;
          effectiveRubyTextStyle = effectiveRubyTextStyle.merge(
            TextStyle(letterSpacing: newLetterSpacing),
          );
        } else {
          final double newLetterSpacing = (rubyWidth - textWidth) / text.length;
          effectiveTextStyle = effectiveTextStyle.merge(
            TextStyle(letterSpacing: newLetterSpacing),
          );
        }
      }

      // ignore: always_specify_types
      return Tuple2(effectiveTextStyle, effectiveRubyTextStyle);
    }, <Object?>[defaultTextStyle, boldTextOverride, data]);

    final TextStyle effectiveTextStyle = result.item1;
    final TextStyle effectiveRubyTextStyle = result.item2;

    final List<Widget> texts = <Widget>[];

    final double rubyWidth = (data.ruby != null && data.ruby != '')
        ? _measurementWidth(data.ruby!, effectiveRubyTextStyle)
        : 0.0;
    final double textWidth = _measurementWidth(data.text, effectiveTextStyle);

    if (data.ruby != null) {
      final String rubyText = data.ruby!.trim();
      texts.add(
        Container(
          color: rubyText.isEmpty
              ? fullOverlay
                  ? effectiveRubyTextStyle.backgroundColor
                  : const Color(0xffffffff)
              : effectiveRubyTextStyle.backgroundColor,
          width: max(rubyWidth, textWidth),
          child: Text(
            data.ruby!,
            textAlign: TextAlign.center,
            style: effectiveRubyTextStyle,
          ),
        ),
      );
    }

    texts.add(
      Container(
        color: effectiveTextStyle.backgroundColor,
        width: max(rubyWidth, textWidth),
        child: Text(
          data.text,
          textAlign: TextAlign.center,
          style: effectiveTextStyle,
        ),
      ),
    );

    return Column(children: texts);
  }
}

class RubyText extends StatelessWidget {
  const RubyText(
    this.data, {
    super.key,
    this.spacing = 0.0,
    this.style,
    this.rubyStyle,
    this.textAlign,
    this.textDirection,
    this.softWrap,
    this.overflow,
    this.maxLines,
    this.fullOverlay = true,
  });

  final List<RubyTextData> data;
  final double spacing;
  final TextStyle? style;
  final TextStyle? rubyStyle;
  final TextAlign? textAlign;
  final TextDirection? textDirection;
  final bool? softWrap;
  final TextOverflow? overflow;
  final int? maxLines;
  final bool fullOverlay;

  @override
  Widget build(BuildContext context) => Text.rich(
        TextSpan(
          children: data
              .map<InlineSpan>(
                (RubyTextData data) => WidgetSpan(
                  child: RubySpanWidget(
                    data.copyWith(
                      style: style,
                      rubyStyle: rubyStyle,
                      textDirection: textDirection,
                    ),
                    fullOverlay: fullOverlay,
                  ),
                ),
              )
              .toList(),
        ),
        textAlign: textAlign,
        textDirection: textDirection,
        softWrap: softWrap,
        overflow: overflow,
        maxLines: maxLines,
      );
}

double _measurementWidth(
  String text,
  TextStyle style, {
  TextDirection textDirection = TextDirection.ltr,
}) {
  final TextPainter textPainter = TextPainter(
    text: TextSpan(text: text, style: style),
    textDirection: textDirection,
    textAlign: TextAlign.center,
  )..layout();
  return textPainter.width;
}
